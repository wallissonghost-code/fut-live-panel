(() => {
  ...
  function applyTransform(){const img=stageImg();if(!img||!image)return;const w=naturalW*baseScale,h=naturalH*baseScale;img.style.width=`${w}px`;img.style.height=`${h}px`;img.style.transform=`translate(-50%,-50%) translate(${offsetX}px,${offsetY}px) scale(${zoom})`;updateZoomLabel()}
  ...
  function loadSource(src,mode='cover'){
    source=src||'';const img=stageImg(),empty=$('#cropEmpty');
    ...
    loader.onload=()=>{image=loader;naturalW=loader.naturalWidth;naturalH=loader.naturalHeight;if(img){img.src=src;img.hidden=false}if(empty)empty.hidden=true;requestAnimationFrame(()=>computeScale(mode))};
  ...
  function switchSide(num){activeSide=num;document.querySelectorAll('[data-crop-side]').forEach(b=>b.classList.toggle('active',Number(b.dataset.cropSide)===num));const s=side();const name=$('#cropCurrentName');if(name)name.innerHTML=`Editando <span>${esc(s?.name||`Lado ${num}`)}</span>`;loadSource(s?.image||'','cover')}
  ...
  function pointerDown(e){if(!image)return;dragging=true;lastX=e.clientX;lastY=e.clientY;stage()?.setPointerCapture?.(e.pointerId);e.preventDefault()}
  function pointerMove(e){if(!dragging||!image)return;offsetX+=e.clientX-lastX;offsetY+=e.clientY-lastY;lastX=e.clientX;lastY=e.clientY;applyTransform();e.preventDefault()}
  function pointerUp(e){dragging=false;stage()?.releasePointerCapture?.(e.pointerId)}
  function saveCrop(){
    if(!image)return alert('Escolha uma imagem primeiro.');
    const box=stage();if(!box)return;
    const outW=800,outH=1000,ratioX=outW/box.clientWidth,ratioY=outH/box.clientHeight;
  ...
  function open(){
    ...
    const box=stage();box?.addEventListener('pointerdown',pointerDown);box?.addEventListener('pointermove',pointerMove);box?.addEventListener('pointerup',pointerUp);box?.addEventListener('pointercancel',pointerUp);
